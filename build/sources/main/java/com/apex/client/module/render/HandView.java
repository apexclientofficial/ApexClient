...
public class HandView extends Module {
    ...
    public void onTick() {
        ...
        if (noSwing.isEnabled()) {
            mc.thePlayer.swingProgress = 0f;
            mc.thePlayer.prevSwingProgress = 0f;
            if (isSwingInProgressField != null) {
                try {
                    isSwingInProgressField.setBoolean(mc.thePlayer, false);
                } catch (Exception ignored) {}
            }
            if (swingProgressIntField != null) {
                try {
                    swingProgressIntField.setInt(mc.thePlayer, 0);
                } catch (Exception ignored) {}
            }
            return;
        }
        ...
        try {
            int currentSwing = swingProgressIntField.getInt(mc.thePlayer);
            if (mc.thePlayer.isSwingInProgress && currentSwing >= 0) {
                float vanillaProgress = (float) currentSwing / 6f;
                int scaledSwing = (int) (vanillaProgress * customDuration);
                
                if (vanillaProgress < 1.0f) {
                    mc.thePlayer.swingProgress = (float) scaledSwing / (float) customDuration;
                    mc.thePlayer.prevSwingProgress = Math.max(0, mc.thePlayer.swingProgress - (1.0f / customDuration));
                }
            }
        } catch (Exception ignored) {}
    }
