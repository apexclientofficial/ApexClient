...
public class Config extends Module {
    ...
    public void onTick() {
        if (save.isEnabled()) {
            ...
            save.setValue(false);
            if (mc.thePlayer != null) mc.thePlayer.addChatMessage(new net.minecraft.util.ChatComponentText("\u00a7c[APEX]\u00a7f Config Saved as '" + configName.getValue() + "'!"));
        }
        if (load.isEnabled()) {
            ApexClient.instance.configManager.load(configName.getValue());
            load.setValue(false);
            if (mc.thePlayer != null) mc.thePlayer.addChatMessage(new net.minecraft.util.ChatComponentText("\u00a7c[APEX]\u00a7f Config Loaded from '" + configName.getValue() + "'!"));
        }
    }
}
